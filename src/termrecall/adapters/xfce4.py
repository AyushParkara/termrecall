...
# Xfce4-Terminal launch must return promptly so restore requests cannot hang forever.
...
class Xfce4TerminalAdapter:
    ...
    def detect(self) -> bool:
        return self._which("xfce4-terminal") is not None
    ...
    def plan(self, items: Sequence[LaunchItem]) -> Sequence[LaunchAction]:
        executable = self._which("xfce4-terminal")
        ...
        for index, item in enumerate(items):
            ...
            if item.approved_command is not None:
                argv += ("--hold", "bash", "-c", _COMMAND_WRAPPER, "bash", *shlex.split(item.approved_command))
