...
from termrecall.adapters.gnome import DEFAULT_LAUNCH_TIMEOUT, GnomeTerminalAdapter
...
def test_detect_uses_injected_executable_resolver() -> None:
    ...
    assert GnomeTerminalAdapter(which).detect()
    assert requested == ["gnome-terminal"]
...
def test_missing_terminal_executable_plans_unavailable_without_argv(
    ...
    action = GnomeTerminalAdapter(lambda _: None).plan(
        [LaunchItem("item-a", tmp_path, None)]
    )[0]
...
def test_execute_skips_preplanned_unavailable_action(tmp_path: Path) -> None:
    ...
    action = GnomeTerminalAdapter(lambda _: None, runner=runner).plan(
        [LaunchItem("item-a", tmp_path, None)]
    )[0]
    outcome = GnomeTerminalAdapter(lambda _: None, runner=runner).execute(
        [action], "attempt-a"
    )[0]
