...
from termrecall.adapters.konsole import DEFAULT_LAUNCH_TIMEOUT, KonsoleAdapter
...
def test_adapter_name_is_konsole() -> None:
    assert KonsoleAdapter(lambda _: EXECUTABLE).name == "konsole"
...
def test_detect_uses_injected_executable_resolver() -> None:
    ...
    assert KonsoleAdapter(which).detect()
    assert requested == ["konsole"]
...
def test_missing_terminal_executable_plans_unavailable_without_argv(
    ...
    action = KonsoleAdapter(lambda _: None).plan(
        [LaunchItem("item-a", tmp_path, None)]
    )[0]
...
def test_execute_skips_preplanned_unavailable_action(tmp_path: Path) -> None:
    ...
    action = KonsoleAdapter(lambda _: None, runner=runner).plan(
        [LaunchItem("item-a", tmp_path, None)]
    )[0]
    outcome = KonsoleAdapter(lambda _: None, runner=runner).execute(
        [action], "attempt-a"
    )[0]
...
def test_rejects_non_positive_launch_timeout() -> None:
    with pytest.raises(ValueError, match="launch timeout must be positive"):
        KonsoleAdapter(lambda _: EXECUTABLE, launch_timeout=0)
